...
Future<CursorPasteResult> pasteIntoCursorComposer(
    {bool autoSend = false}) async {
  if (!Platform.isMacOS) {
    return CursorPasteResult(ok: false, detail: 'macOS only');
  }
  try {
    await _channel.invokeMethod('pasteClipboard',
        {'autoSend': autoSend}).timeout(const Duration(seconds: 8));
