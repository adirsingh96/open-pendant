...
*   **IMU:** LSM6DS3TR-C. Firmware sleeps the PDM mic when the board is **still** (~10 s), wakes on motion. Mic noise is ignored here; the host VAD drops quiet audio before STT. Not a consent signal — notify + LED still mean recording.
*   **Power:** 3.7 V 300 mAh 1S LiPo (WLY602030); USB for development
...
*   **Phone:** Flutter app in [`app/`](app/) — BLE PCM → WAV → OpenAI → SQLite
...
*   **Status UUID:** `70301103-4a1b-4c8d-9e0f-a1b2c3d4e5f6` (READ/NOTIFY)
    *   byte 0: flags (`1` IMU sleep, `2` mic running, `4` IMU ready, `8` last accel fetch ok)
    *   bytes 1–2: LE volume
...
Notify-on = recording. Phone stores `started_at` (UTC) at that instant. On stop: WAV → OpenAI `verbose_json` → SQLite.

*   **STT:** `gpt-4o-mini-transcribe`, fallback `whisper-1`. Key on the phone (Keychain/Keystore), never on the MCU.
*   **clips:** `id`, `started_at`, `duration_s`, `full_text`, `wav_path`, `stt_model`, `status`
*   **segments:** `start_s`, `end_s`, `spoken_at` (= `started_at + start_s`), `text`
...
*   Always-on capture, IMU tap / wear gate, hardware button
*   Companion / live assistant

## OSS notes
*   **BOM:** XIAO nRF52840 Sense + optional 1S LiPo + USB-C cable
*   **License:** Apache-2.0
*   **Closest cousin:** [Omi](https://github.com/BasedHardware/omi) — larger nRF/Flutter/cloud stack. OpenPendant stays a small XIAO Sense + documented protocol.
